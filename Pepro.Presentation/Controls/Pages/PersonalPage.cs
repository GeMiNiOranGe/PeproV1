...
public partial class PersonalPage : MediatedTemplate
{
    ...
    private void PersonalPage_Load(object sender, EventArgs e)
    {
        EmployeeDto? employee =
            EmployeeBusiness.Instance.GetEmployeeByEmployeeId(_employeeId);
        if (employee == null)
        {
        ...
        DepartmentDto? department =
            DepartmentBusiness.Instance.GetDepartmentByDepartmentId(
        ...
        string[] projectNames =
            ProjectBusiness.Instance.GetProjectNamesByEmployeeId(_employeeId);
        assignedProjectsComboBoxField.DataSource =
            projectNames.Length != 0 ? projectNames : ["Không tìm thấy"];

        string[] phoneNumbers =
            EmployeeBusiness.Instance.GetPhoneNumbersByEmployeeId(_employeeId);
        phoneNumberComboBoxField.DataSource =
            phoneNumbers.Length != 0 ? phoneNumbers : ["Không tìm thấy"];
    }
}
